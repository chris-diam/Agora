  ...
  return (
    <div className="relative">
      <button
        ...
        className="relative flex h-9 w-9 items-center justify-center rounded-full text-gray-500 hover:bg-gray-900/5"
      >
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 z-30 mt-2 w-80 overflow-hidden rounded-2xl border border-white/60 bg-white/95 shadow-lg shadow-gray-900/10 backdrop-blur-xl">
            ...
            <div className="max-h-96 overflow-y-auto">
              ...
              {!isLoading && items.length === 0 && (
                <p className="px-4 py-3 text-sm text-gray-400">No notifications yet.</p>
              )}
              {items.map((notification) => {
                const isFollowingBack = notification.followingBack || followedBackIds.has(notification.actor.id);
                return (
                  <div key={notification.id} className="flex items-center gap-3 px-4 py-3 text-sm hover:bg-gray-900/5">
                    <Link
                    ...
                    >
                      ...
                      <span className="min-w-0 truncate">
                        <span className="font-medium text-gray-900">{notification.actor.displayName}</span>{" "}
                        <span className="text-gray-500">started following you</span>
                      </span>
                    </Link>
                    {isFollowingBack ? (
                      <span className="shrink-0 text-xs text-gray-400">Following</span>
                    ) : (
                      <button
                        type="button"
